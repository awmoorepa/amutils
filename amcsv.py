...
import arrays
import basic
from arrays import Strings, StringsArray
from basic import Character, Errmess
...
def trim_string(s: str) -> str:
    ...
    for c in s:
        if c == ' ':
            ...
            is_started = True
            result = result + basic.n_spaces(n_spaces_since_non_space) + c
            n_spaces_since_non_space = 0
...
class CsvReader:
    def __init__(self, state: Cstate, word: str, row: arrays.Strings, em: basic.Errmess):
        self.m_cstate = state
        self.m_word = word
        self.m_row = row
        self.m_errmess = em
    ...
    def row(self) -> arrays.Strings:
        return self.m_row

    def errmess(self) -> basic.Errmess:
    ...
    def emit_char(self, c: basic.Character, next_state: Cstate):
    ...
    def emit_error(self, message: str):
        assert message != ""
        self.m_errmess = basic.errmess_error(message)
...
def csv_reader_start() -> CsvReader:
    return CsvReader(Cstate.begin_line, "", arrays.strings_empty(), basic.errmess_ok())


class CsvRowOutput:
    def __init__(self, em: basic.Errmess, row: arrays.Strings, file_has_ended: int):
        self.m_errmess = em
        self.m_row = row
    ...
    def assert_ok(self):
        ...
        assert isinstance(self.m_row, arrays.Strings)
...
def csv_row_output_type_errmess(em: basic.Errmess) -> CsvRowOutput:
    return CsvRowOutput(em, arrays.strings_empty(), -77)


def csv_row_output_type_ok(row: arrays.Strings, file_has_ended: bool) -> CsvRowOutput:
    return CsvRowOutput(basic.errmess_ok(), row, file_has_ended)


def csv_row_output_file_ended_without_row() -> CsvRowOutput:
    return csv_row_output_type_ok(arrays.strings_empty(), True)


def csv_row_output_file_ended_with_row(row: arrays.Strings):
    return csv_row_output_type_ok(row, True)


def csv_row_output_ok_and_file_not_ended(row: arrays.Strings):
    return csv_row_output_type_ok(row, False)


class StringStream:
    def __init__(self, ss: arrays.Strings):
        self.m_strings = ss
        self.m_line_num = 0
        self.m_char_num = 0
    ...
    def assert_ok(self):
        assert isinstance(self.m_strings, arrays.Strings)
    ...
    def next_character(self) -> Union[tuple[None, bool], tuple[Character, bool]]:
        if self.m_end_of_file:
            return None, False
        else:
            ...
            if self.m_char_num == len(s):
                ...
                return basic.character_newline(), True
            else:
                result = basic.character_from_string(s, self.m_char_num)
                self.m_char_num += 1
                return result, True
...
def string_stream_from_strings_array(ss: arrays.Strings) -> StringStream:
...
def next_csv_row_output(ss: StringStream) -> Union[
        tuple[None, Errmess, NextRowStatus], tuple[Strings, Errmess, NextRowStatus]]:
    cs = csv_reader_start()
    ...
    while not cs.is_end_of_line():
        ...
        if cs.state_is(Cstate.begin_line):
            ...
            basic.my_error("We can't find ourselves here")
        elif cs.state_is(Cstate.end_line):
            basic.my_error("We can't find ourselves here")
        elif cs.state_is(Cstate.error):
            basic.my_error("We can't find ourselves here")
        else:
            basic.my_error("Bad CSV state")

    if cs.state_is(Cstate.error):
        return None, cs.errmess(), NextRowStatus.error
    elif cs.state_is(Cstate.finish):
        if cs.row().len() == 0:
            return None, basic.errmess_ok(), NextRowStatus.file_ended_without_extra_row
        else:
            return cs.row(), basic.errmess_ok(), NextRowStatus.file_ended_with_extra_row
    else:
        assert cs.state_is(Cstate.end_line)
        return cs.row(), basic.errmess_ok(), NextRowStatus.maybe_more_lines_to_come


def strings_array_from_string_stream_csv(sts: StringStream) -> Union[
        tuple[None, Errmess], tuple[StringsArray, Errmess]]:
...
def strings_array_from_strings_csv(ss: arrays.Strings) -> Union[tuple[None, Errmess], tuple[StringsArray, Errmess]]:
